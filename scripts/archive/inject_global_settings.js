...
fs.writeFileSync(path.join(publicDir, 'global_settings.js'), globalScript);

// 2. Inject into all HTML files except index.html
const files = fs.readdirSync(publicDir).filter(f => f.endsWith('.html'));

files.forEach(file => {
    if (file === 'index.html') return; // index.html has its own openSettings modal built in

    let filePath = path.join(publicDir, file);
    let html = fs.readFileSync(filePath, 'utf8');

    // Make the sidebar link call openGlobalSettings()
    // Find <a ...> ... <span class="nav-text">Settings</span></a>
    html = html.replace(/<a onclick="window\.location\.href='\/'"([^>]*)>([^<]*)<span class="nav-text">Settings<\/span><\/a>/g, '<a onclick="openGlobalSettings()"$1 style="cursor:pointer; margin-top: 15px; border-top: 1px solid #2f3542; border-radius: 0; padding-top: 15px;">$2<span class="nav-text">Settings</span></a>');
    
    // Some files might still have onclick="openSettings()" from before my last script (if I missed them or they were skipped)
    html = html.replace(/<a onclick="openSettings\(\)"([^>]*)>([^<]*)<span class="nav-text">Settings<\/span><\/a>/g, '<a onclick="openGlobalSettings()"$1>$2<span class="nav-text">Settings</span></a>');

    // Also the standalone Settings buttons
    html = html.replace(/<button class="settings-btn" onclick="window\.location\.href='\/'"/g, '<button class="settings-btn" onclick="openGlobalSettings()"');

    // And make sure the script is included at the end of body
    if (!html.includes('<script src="/global_settings.js"></script>')) {
        html = html.replace('</body>', '    <script src="/global_settings.js"></script>\n</body>');
    }

    // Clean up any rogue script tag redirecting openSettings to /
    html = html.replace(/<script>function openSettings\(\) { window\.location\.href = '\/'; }<\/script>/g, '');

    fs.writeFileSync(filePath, html);
});

console.log('Injected global_settings.js into all subpages');
