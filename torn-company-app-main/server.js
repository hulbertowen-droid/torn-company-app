...
app.post('/api/ai-analyze', async (req, res) => {
    if (!GEMINI_API_KEY) return res.status(500).json({error: "No Gemini API key configured."});
    res.json({success: true, analysis: "AI Analysis feature is currently under development."});
});

app.post('/api/ai-analyze-ongoing', async (req, res) => {
    if (!GEMINI_API_KEY) return res.status(500).json({error: "No Gemini API key configured."});
    res.json({success: true, analysis: "Live War AI Analysis feature is currently under development."});
});

app.post('/api/generate-recruit-msg', async (req, res) => {
    if (!GEMINI_API_KEY) return res.status(500).json({error: "No Gemini API key configured."});
    res.json({success: true, message: "Hey there! We noticed your solid stats in the recent war. Our faction is currently recruiting active players like you. Let us know if you're looking for a new home!"});
});
...
app.get('/api/chain-data', async (req, res) => {
    ...
    try {
        await verifySubscription(userKey);
        const activeKey = getNextApiKey() || userKey;
        const facRes = await fetch(`https://api.torn.com/faction/?selections=basic,chain&key=${activeKey}`);
        const facData = await facRes.json();
        if (facData.error) return res.status(400).json({ error: facData.error.error });
        res.json(facData);
    } catch (err) { res.status(403).json({ error: err.message }); }
...
app.get('/api/chain-ledger', async (req, res) => {
    const userKey = req.query.apiKey;
    try {
        await verifySubscription(userKey);
        const activeKey = getNextApiKey() || userKey;
        const facRes = await fetch(`https://api.torn.com/faction/?selections=attacks&key=${activeKey}`);
        const facData = await facRes.json();
        if (facData.error) return res.status(400).json({ error: facData.error.error });
        res.json(facData);
    } catch (err) { res.status(403).json({ error: err.message }); }
...
app.get('/api/torn-items', async (req, res) => {
    const userKey = req.query.apiKey;
    try {
        await verifySubscription(userKey);
        const now = Date.now();
        if (tornItemsCache && (now - tornItemsCacheTime < 3600000)) {
            return res.json(tornItemsCache);
        }
        const activeKey = getNextApiKey() || userKey;
        const itemRes = await fetch(`https://api.torn.com/torn/?selections=items&key=${activeKey}`);
        const itemData = await itemRes.json();
        if (itemData.error) return res.status(400).json({ error: itemData.error.error });
        
        tornItemsCache = itemData;
        tornItemsCacheTime = now;
        res.json(itemData);
    } catch (err) { res.status(403).json({ error: err.message }); }
});
